...
class Program
{
    ...
    private static async Task TestConnectionsCommand(
        MigrationService migrationService,
        ILogger<Program> logger
    )
    {
        ...
        {
            Console.WriteLine(
                "❌ Database connection tests failed. Check your configuration and ensure databases are running."
            );
        }
    }

    private static async Task MigrateCommand(
        MigrationService migrationService,
        ILogger<Program> logger
    )
    {
        ...
        Console.WriteLine(
            "Make sure your MongoDB database is empty or you want to overwrite existing data."
        );
