...
class Program
{
    ...
    private static async Task TestConnectionsCommand(IServiceProvider serviceProvider)
    {
        ...
        var migrationOrchestrator =
            scope.ServiceProvider.GetRequiredService<IMigrationOrchestrator>();
    ...
    private static async Task MigrateCommand(IServiceProvider serviceProvider)
    {
        ...
        try
        {
            ...
            Console.WriteLine(
                "  MATCH (m:Media)-[:BELONGS_TO_GENRE]->(g:Genre) RETURN m.name, g.name"
            );
            Console.WriteLine(
                "  MATCH (u:User)-[:SUBSCRIBES_TO]->(s:Subscription)-[:GIVES_ACCESS_TO]->(g:Genre) RETURN u.firstName, s.name, collect(g.name)"
            );
        }
        catch (Exception ex)
        {
