...
public interface IPostgreSqlDataService
{
    ...
    Task<IEnumerable<(int ProfileId, int MediaId, int Rating, string ReviewText, DateTime ReviewDate)>> GetReviewsAsync();
...
public class PostgreSqlDataService : IPostgreSqlDataService
{
    ...
    public PostgreSqlDataService(IConfiguration configuration, ILogger<PostgreSqlDataService> logger)
    {
        _logger = logger;
    ...
    public async Task<IEnumerable<(int UserId, int ProfileId)>> GetUserProfileRelationshipsAsync()
    {
        ...
        var relationships = await _context.Profiles
    ...
    public async Task<IEnumerable<(int ProfileId, int WatchListId)>> GetProfileWatchListsAsync()
    {
        ...
        var relationships = await _context.WatchLists
            .Select(w => new { ProfileId = w.ProfileId, WatchListId = w.ProfileId }) // WatchList uses ProfileId as both FK and PK
            .ToListAsync();
    ...
    public async Task<IEnumerable<(int WatchListId, int MediaId)>> GetWatchListMediasAsync()
    {
        ...
        try
        {
            var relationships = await _context.WatchLists
                .Include(w => w.Medias)
                .SelectMany(w => w.Medias.Select(m => new { WatchListId = w.ProfileId, MediaId = m.Id }))
                .ToListAsync();
            
            _logger.LogInformation($"Retrieved {relationships.Count} watchlist-media relationships");
            return relationships.Select(r => (r.WatchListId, r.MediaId));
        }
        catch (Exception ex)
        {
            _logger.LogWarning(ex, "Could not fetch watchlist-media relationships, returning empty collection");
    ...
    public async Task<IEnumerable<(int MediaId, int EpisodeId)>> GetMediaEpisodesAsync()
    {
        ...
        var relationships = await _context.Episodes
            .Select(e => new { e.MediaId, e.Id })
            .ToListAsync();
    ...
    public async Task<IEnumerable<(int MediaId, int GenreId)>> GetMediaGenresAsync()
    {
        ...
        try
        {
            var relationships = await _context.Medias
                .Include(m => m.Genres)
        ...
        {
            _logger.LogWarning(ex, "Could not fetch media-genre relationships, returning empty collection");
            return new List<(int, int)>();
        }
    }

    public async Task<IEnumerable<(int PersonId, int MediaId, string Role)>> GetPersonMediaRolesAsync()
    {
        _logger.LogInformation("Fetching person-media-role relationships...");
        
        try
        {
            var relationships = await _context.MediaPersonRoles
                .Include(mpr => mpr.Person)
                .Include(mpr => mpr.Media)
                .Include(mpr => mpr.Role)
                .Select(mpr => new { mpr.PersonId, mpr.MediaId, RoleName = mpr.Role.Name })
                .ToListAsync();
            
            _logger.LogInformation($"Retrieved {relationships.Count} person-media-role relationships");
            return relationships.Select(r => (r.PersonId, r.MediaId, r.RoleName));
        }
        catch (Exception ex)
        {
            _logger.LogWarning(ex, "Could not fetch person-media-role relationships, returning empty collection");
            return new List<(int, int, string)>();
        }
    }

    public async Task<IEnumerable<(int ProfileId, int MediaId, int Rating, string ReviewText, DateTime ReviewDate)>> GetReviewsAsync()
    {
        _logger.LogInformation("Fetching profile review relationships...");
        
        try
        {
            var relationships = await _context.Reviews
                .Select(r => new { r.ProfileId, r.MediaId, r.Rating, Description = r.Description, r.CreatedAt })
                .ToListAsync();
            
            _logger.LogInformation($"Retrieved {relationships.Count} profile review relationships");
            return relationships.Select(r => (r.ProfileId, r.MediaId, r.Rating, r.Description ?? "", r.CreatedAt));
        }
        catch (Exception ex)
        {
            _logger.LogWarning(ex, "Could not fetch profile review relationships, returning empty collection");
    ...
    public async Task<IEnumerable<(int UserId, int SubscriptionId)>> GetUserSubscriptionsAsync()
    {
        ...
        try
        {
            var relationships = await _context.Users
                .Include(u => u.Subscriptions)
                .SelectMany(u => u.Subscriptions.Select(s => new { UserId = u.Id, SubscriptionId = s.Id }))
                .ToListAsync();
            
            _logger.LogInformation($"Retrieved {relationships.Count} user-subscription relationships");
            return relationships.Select(r => (r.UserId, r.SubscriptionId));
        }
        catch (Exception ex)
        {
            _logger.LogWarning(ex, "Could not fetch user-subscription relationships, returning empty collection");
            return new List<(int, int)>();
        }
    }
    ...
    public async Task<IEnumerable<(int UserId, int PrivilegeId)>> GetUserPrivilegesAsync()
    {
        ...
        try
        {
            var relationships = await _context.Users
                ...
                .SelectMany(u => u.Privileges.Select(p => new { UserId = u.Id, PrivilegeId = p.Id }))
                .ToListAsync();
        ...
        {
            _logger.LogWarning(ex, "Could not fetch user-privilege relationships, returning empty collection");
            return new List<(int, int)>();
        }
    }
