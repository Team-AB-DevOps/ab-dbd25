...
public class MigrationOrchestrator : IMigrationOrchestrator
{
    ...
    public async Task RunMigrationAsync()
    {
        ...
        try
        {
            ...
            await Task.WhenAll(usersTask, profilesTask, watchListsTask, mediasTask, episodesTask,
                              personsTask, genresTask, rolesTask, subscriptionsTask, privilegesTask);
            ...
            _logger.LogInformation("Extracted {UserCount} users, {ProfileCount} profiles, {WatchListCount} watchlists, {MediaCount} media items", 
                users.Count(), profiles.Count(), watchLists.Count(), medias.Count());
            ...
            await _neo4jService.MigrateNodesAsync(graphUsers, graphProfiles, graphWatchLists, graphMedias, graphEpisodes,
                                                 graphPersons, graphGenres, graphRoles, graphSubscriptions, graphPrivileges);
            ...
            await Task.WhenAll(userProfilesTask, profileWatchListsTask, watchListMediasTask, mediaEpisodesTask, mediaGenresTask,
                              personMediaRolesTask, reviewsTask, userSubscriptionsTask, subscriptionGenresTask, userPrivilegesTask);
            ...
            _logger.LogInformation("Extracted {UserProfileCount} user-profile, {ReviewCount} review, {PersonMediaRoleCount} person-media-role relationships", 
                userProfiles.Count(), reviews.Count(), personMediaRoles.Count());

            // Step 7: Migrate relationships to Neo4j
            _logger.LogInformation("Step 7: Migrating relationships to Neo4j...");
            await _neo4jService.MigrateRelationshipsAsync(userProfiles, profileWatchLists, watchListMedias, mediaEpisodes,
                                                         mediaGenres, personMediaRoles, reviews, userSubscriptions,
                                                         subscriptionGenres, userPrivileges);

            var endTime = DateTime.UtcNow;
            var duration = endTime - startTime;

            // Log migration statistics
            _logger.LogInformation("🎉 Migration completed successfully in {Duration:hh\\:mm\\:ss}", duration);
            ...
            _logger.LogInformation("    💳 Subscriptions: {SubscriptionCount}", graphSubscriptions.Count);
            _logger.LogInformation("    🔐 Privileges: {PrivilegeCount}", graphPrivileges.Count);
            _logger.LogInformation("  🔗 Relationships migrated:");
            _logger.LogInformation("    👥➡️🎭 User-Profile: {UserProfileCount}", userProfiles.Count());
            _logger.LogInformation("    🎭➡️📝 Profile-WatchList: {ProfileWatchListCount}", profileWatchLists.Count());
            _logger.LogInformation("    📝➡️🎬 WatchList-Media: {WatchListMediaCount}", watchListMedias.Count());
            _logger.LogInformation("    🎬➡️📺 Media-Episode: {MediaEpisodeCount}", mediaEpisodes.Count());
            _logger.LogInformation("    🎬➡️🏷️ Media-Genre: {MediaGenreCount}", mediaGenres.Count());
            _logger.LogInformation("    🎭➡️🎬 Person-Media-Role: {PersonMediaRoleCount}", personMediaRoles.Count());
            _logger.LogInformation("    🎭➡️🎬 Reviews: {ReviewCount}", reviews.Count());
            _logger.LogInformation("    👥➡️💳 User-Subscription: {UserSubscriptionCount}", userSubscriptions.Count());
            _logger.LogInformation("    💳➡️🏷️ Subscription-Genre: {SubscriptionGenreCount}", subscriptionGenres.Count());
            _logger.LogInformation("    👥➡️🔐 User-Privilege: {UserPrivilegeCount}", userPrivileges.Count());
        }
        catch (Exception ex)
        {
