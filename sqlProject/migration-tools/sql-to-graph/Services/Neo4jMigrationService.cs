...
public class Neo4jMigrationService : INeo4jMigrationService, IDisposable
{
    ...
    private async Task MigrateProfilesAsync(IEnumerable<GraphProfile> profiles)
    {
        ...
        var profileDictionaries = profiles
    ...
    private async Task MigrateWatchListsAsync(IEnumerable<GraphWatchList> watchLists)
    {
        ...
        var watchListDictionaries = watchLists
    ...
    private async Task MigrateEpisodesAsync(IEnumerable<GraphEpisode> episodes)
    {
        ...
        var episodeDictionaries = episodes
    ...
    private async Task MigratePersonsAsync(IEnumerable<GraphPerson> persons)
    {
        ...
        var personDictionaries = persons
            .Select(p => new Dictionary<string, object>
            {
                ["id"] = p.Id,
    ...
    private async Task MigrateGenresAsync(IEnumerable<GraphGenre> genres)
    {
        ...
        var genreDictionaries = genres
            .Select(g => new Dictionary<string, object> { ["id"] = g.Id, ["name"] = g.Name })
            .ToList();
    ...
    private async Task MigrateRolesAsync(IEnumerable<GraphRole> roles)
    {
        ...
        var roleDictionaries = roles
            .Select(r => new Dictionary<string, object> { ["id"] = r.Id, ["name"] = r.Name })
            .ToList();
    ...
    private async Task MigrateSubscriptionsAsync(IEnumerable<GraphSubscription> subscriptions)
    {
        ...
        var subscriptionDictionaries = subscriptions
        ...
        await ExecuteBatchedCypher(
            cypher,
            subscriptionDictionaries,
            "subscriptions",
            "Subscription"
        );
    }
    ...
    private async Task MigratePrivilegesAsync(IEnumerable<GraphPrivilege> privileges)
    {
        ...
        var privilegeDictionaries = privileges
    ...
    private async Task MigrateUserProfileRelationshipsAsync(
        IEnumerable<(int UserId, int ProfileId)> relationships
    )
    {
        const string cypher =
            @"
    ...
    private async Task MigrateProfileWatchListRelationshipsAsync(
        IEnumerable<(int ProfileId, int WatchListId)> relationships
    )
    {
        const string cypher =
            @"
    ...
    private async Task MigrateWatchListMediaRelationshipsAsync(
        IEnumerable<(int WatchListId, int MediaId)> relationships
    )
    {
        const string cypher =
            @"
    ...
    private async Task MigrateMediaEpisodeRelationshipsAsync(
        IEnumerable<(int MediaId, int EpisodeId)> relationships
    )
    {
        const string cypher =
            @"
    ...
    private async Task MigrateMediaGenreRelationshipsAsync(
        IEnumerable<(int MediaId, int GenreId)> relationships
    )
    {
        const string cypher =
            @"
    ...
    private async Task MigratePersonMediaRoleRelationshipsAsync(
        IEnumerable<(int PersonId, int MediaId, string Role)> relationships
    )
    {
        const string cypher =
            @"
    ...
    private async Task MigrateReviewRelationshipsAsync(
        IEnumerable<(
            int ProfileId,
            int MediaId,
            int Rating,
    ...
    )
    {
        ...
        var data = relationships
            .Select(r => new Dictionary<string, object>
            {
                ...
                ["reviewDate"] = r.ReviewDate.ToString(
    ...
    private async Task MigrateUserSubscriptionRelationshipsAsync(
        IEnumerable<(int UserId, int SubscriptionId)> relationships
    )
    {
        const string cypher =
            @"
    ...
    private async Task MigrateSubscriptionGenreRelationshipsAsync(
        IEnumerable<(int SubscriptionId, int GenreId)> relationships
    )
    {
        const string cypher =
            @"
    ...
    private async Task MigrateUserPrivilegeRelationshipsAsync(
        IEnumerable<(int UserId, int PrivilegeId)> relationships
    )
    {
        const string cypher =
            @"
    ...
    private async Task ExecuteBatchedCypher<T>(
        string cypher,
        List<T> data,
    ...
    )
    {
        ...
        _logger.LogInformation(
            "Migrating {Count} {EntityType} entities...",
            data.Count,
            entityType
        );
        ...
        foreach (var batch in batches)
        {
            ...
            try
            {
                ...
                _logger.LogDebug(
                    "Processed {Processed}/{Total} {EntityType} entities",
                    totalProcessed,
                    data.Count,
                    entityType
                );
            ...
            catch (Exception ex)
            {
                _logger.LogError(
                    ex,
                    "Failed to migrate batch of {EntityType} entities",
                    entityType
                );
                throw;
            }
        ...
        _logger.LogInformation(
            "Successfully migrated {Count} {EntityType} entities",
            data.Count,
            entityType
        );
    }
    ...
    private async Task ExecuteBatchedCypher(
        string cypher,
