...
public class MigrationService
{
    ...
    public async Task RunFullMigrationAsync()
    {
        _logger.LogInformation(
            "Starting full migration from PostgreSQL to MongoDB with embedded documents..."
        );
    ...
    private async Task MigrateSubscriptionsAsync()
    {
        ...
        var mongoSubscriptions = subscriptions
            .Select(s => new MongoSubscription
            {
                Id = s.Id,
                Name = s.Name,
                Price = s.Price,
    ...
    {
        _logger.LogInformation(
            "Migrating Users with embedded profiles, subscriptions, and privileges..."
        );
        ...
        foreach (var user in users)
        {
            ...
            var profiles = await context.Profiles.Where(p => p.UserId == user.Id).ToListAsync();
            ...
            foreach (var profile in profiles)
            {
                ...
                var reviews = await context
                    .Reviews.Where(r => r.ProfileId == profile.Id)
                    .ToListAsync();
    ...
    private async Task MigrateMediaWithEmbeddedDataAsync()
    {
        ...
        foreach (var media in medias)
        {
            ...
            var credits = media
                .MediaPersonRoles.GroupBy(mpr => mpr.PersonId)
    ...
    private async Task MigratePersonsAsync()
    {
        ...
        var mongoPersons = persons
            .Select(p => new MongoPerson
            {
                Id = p.Id,
    ...
    private async Task MigrateEpisodesAsync()
    {
        ...
        var mongoEpisodes = episodes
            .Select(e => new MongoEpisode
            {
                Id = e.Id,
                Name = e.Name,
